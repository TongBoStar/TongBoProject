...
$(function () {
	...
	ue.ready(function() {
		var height = $("#centerform").height();
		height = height - $("#edui1_toolbarbox").height() - 102;
		ue.setHeight(height);
	});
});
	...
	$('#form').form('submit',{
        onSubmit:function(){
            if($(this).form('enableValidation').form('validate')){
            	$.ajax({
    				type: "post",
    				url: "${ctx}/oa/notice/notice!edit.htm",
    				data:$('#form').serialize(),
    				dataType: "json",
    				async: false,
    				beforeSend:function(){},
				...
				<div data-options="region:'center',border:false" id="centerform">
					<form id="form" method="post">
						<table class="my-table-form">
							<tr>
				   				<th class="my-w80">公告标题</th>
				      			<td>
				      				<s:textfield id="title" name="notice.title" theme="simple" cssClass="easyui-textbox" style="width:400px;" type="text" data-options="required:true"></s:textfield>
									<s:hidden id="id" name="notice.id"></s:hidden>
				        		</td>
				        	</tr>
				        	<tr>
				   				<td colspan="2">
				   					<s:textarea id="contents" name="notice.contents" theme="simple" style="width:100%;height:100%"></s:textarea>
				        		</td>
				        	</tr>
						</table>
					</form>
