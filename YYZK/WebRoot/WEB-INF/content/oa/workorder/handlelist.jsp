...
$(document).ready(function() {
    $('#centergrid').datagrid({
    	url:'${ctx}/admin/oa/workorder/oa-work-order!pageList.htm?oaWorkOrderQuery.workOrderType='+GetQueryString("workOrderType"),
        title: '工单列表',
        toolbar: '#toolbar',
        border: true,
        fit: true,
...
function repair(){
	var row = $('#centergrid').datagrid('getSelected');
	 if (row) {
		...
		openWin('处理工单','myicon-repair',600,400,'${ctx}/admin/oa/workorder/handleIndex.htm?oaWorkOrder.id='+row.id,iframeId);
	} else {
