...
var cardview = $.extend({}, $.fn.datagrid.defaults.view, {
    renderRow: function(target, fields, frozen, rowIndex, rowData){
        ...
        if (!frozen && rowData.jcId){
            var aa = rowData.url;
            var img = aa;
            
            cc.push('<img src="${ctx}/upload/jc/' + img + '" style="width:150px;float:left">');
...
function add(){
	...
	if(row){
		...
		$.ajax({
			type: "post",
			url: "${ctx}/offer/offer!projectUpdateJcImg.htm?offerProject.projectId="+GetQueryString("projectId")+"&offerProjectQuery.jcImg="+row.url,
			dataType: "json",
			async: false,
			beforeSend:function(){},
			success: function (data, textStatus) {
				if(data.success){
					window.parent.$("#jcimg").attr('src','${ctx}/upload/jc/'+row.url);
