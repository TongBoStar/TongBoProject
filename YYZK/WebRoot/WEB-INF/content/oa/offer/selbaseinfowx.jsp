...
var cardview = $.extend({}, $.fn.datagrid.defaults.view, {
    renderRow: function(target, fields, frozen, rowIndex, rowData){
        ...
        if (!frozen && rowData.wxId){
            var aa = rowData.url;
            var img = aa;
            
            cc.push('<img src="${ctx}/upload/wx/' + img + '" style="width:150px;float:left">');
...
function add(){
	...
	if(row){
		...
		$.ajax({
			type: "post",
			url: "${ctx}/offer/offer!projectUpdateWxImg.htm?offerProject.projectId="+GetQueryString("projectId")+"&offerProjectQuery.wxImg="+row.url,
			dataType: "json",
			async: false,
			beforeSend:function(){},
			success: function (data, textStatus) {
				if(data.success){
					window.parent.$("#wximg").attr('src','${ctx}/upload/wx/'+row.url);
