...
$(function () {
	$('#centertreegrid').treegrid({
		url: '${ctx}/offer/offer!dictTreeGrid.htm',
		border:false,
	    toolbar: '#toolbar',
	    idField:'dictTreeId',
	    treeField:'name',
...
function edit(){
	...
	if(node){
		...
		$('#win').window({
		    ...
		    maximizable: true, 
		    closable: true,
		    content:'<iframe id="mainWorkArea" name="mainWorkArea" src="${ctx}/offer/dictTreeEditIndex.htm?dictTree.dictTreeId='+node.dictTreeId+'"  broder="0" scrolling="atuo" align="middle" frameborder="0" width="100%" height="100%"></iframe>',
		    onClose: function () {}
		});
	}
}
...
function del(){
	...
	if(node){
		$.ajax({
			type: "post",
			url: "${ctx}/offer/offer!dictTreeDelete.htm?dictTree.dictTreeId="+node.dictTreeId,
			dataType: "json",
			async: false,
			beforeSend:function(){},
