...
$(document).ready(function() {
    $('#centergrid').datagrid({
    	url:'${ctx}/tasktiaoyazhan/task-tiao-ya-zhan!pageList.htm?taskTiaoYaZhanQuery.taskState=${param.taskState}',
        title: '调压器任务列表',
        toolbar: '#toolbar',
        border: true,
        fit: true,
        ...
        frozenColumns:[[
     	...
     	columns: [[
			...
			{ field: 'customerName', title: '单位名称'},
			{ field: 'productModel', title: '产品型号'},
			{ field: 'tiaoYaQiModel', title: '调压器型号'},
			{ field: 'inPressure', title: '进口压力'},
			{ field: 'outPressure', title: '出口压力'},
			{ field: 'filterNumber', title: '过滤器令号或型号'},
			{ field: 'equipmentColor', title: '设备颜色'},
			{ field: 'boxColor', title: '箱体'},
...
function add(){
	window.location.href="${ctx}/tasktiaoyazhan/addIndex.htm";
}

function edit(){
	var row = $('#centergrid').datagrid('getSelected');
	 if (row) {
		 window.location.href="${ctx}/tasktiaoyazhan/addIndex.htm?taskTiaoYaZhan.id="+row.id;
	 }else {
...
function del(){
	...
	if (row) {
		$.ajax({
			type: "post",
			url: "${ctx}/tasktiaoyazhan/task-tiao-ya-zhan!delete.htm",
			data:{"taskTiaoYaZhan.id":row.id},
			dataType: "json",
			async: false,
			success: function (data, textStatus) {
				alert(data.msg);
				window.location.href="${ctx}/oa/task/tiaoyanzhan/list.htm";
			}
		});
	} else {
