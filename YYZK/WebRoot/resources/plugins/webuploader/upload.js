...
(function($) {
	...
	$(function() {
		...
		var domain = src.split('domain=');
		domain = window.location.origin+domain[1];
		...
		uploader = WebUploader.create({
			...
			//  sendAsBinary:true,
			chunkSize : 512 * 1024,
			server : domain+'/fileupload/uploadHeadPic.htm',
			// runtimeOrder: 'flash',
			// accept: {
			//     title: 'Images',
		...
		uploader.on('uploadSuccess', function(file,response) {
			//alert(response);
			var li ='<li class="state-complete">'+
					'<p class="del" style="margin:0px;" onclick="javascript:$(this).parent().remove();">删除</p>'+
					'<p class="imgWrap"><img style="width:110px;height:110px;" data-oldname="'+response.oldName+'" data-newname="'+response.newName+'" data-respath="'+response.resPath+'" src="'+domain+'/'+response.resPath+'"></p>'+
					'</li>';
			$("#isupload").append(li);
		});
