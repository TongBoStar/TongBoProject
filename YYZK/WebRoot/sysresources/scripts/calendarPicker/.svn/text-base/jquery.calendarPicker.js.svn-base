  ...
  calendar.changeDate = function(date) {
    ...
    var fillYears = function(date) {
      ...
      for (var i = year - options.years; i <= year + options.years; i++) {
        var d = new Date(date);
        d.setFullYear(i);
        var span = $("<span>").addClass("calElement").attr("millis", d.getTime()).html(i+"年").css("width",w);
    ...
    var fillDays = function(date) {
      ...
      var t = new Date();
      divDays.empty();
      var nc = options.days*4+1;
      var w = parseInt(-1+(theDiv.width()-4-(options.showDayArrows?12:0)-(nc)*4)/(nc-(options.showDayArrows?2:0)))+"px";
      
      for (var i = -1; i <= (options.days*4); i++) {
        var d = new Date(date);
        d.setDate(day + i)
        var span = $("<span>").addClass("calElement").attr("millis", d.getTime())
        if (i == -1 && options.showDayArrows) {
          span.addClass("prev");
          span.attr("title","向前一周");
        } else if (i == (options.days*4) && options.showDayArrows) {
          span.addClass("next");
          span.attr("title","向后一周");
        } else {
  ...
  theDiv.click(function(ev) {
    ...
    if (el.hasClass("calElement")) {
    	if(el.hasClass("next")){
    		...
    		calendar.changeDate(lastdate);
    	}else if(el.hasClass("prev")){
    	//	lastdate.setDate(lastdate.getDate()-7);
    	//	calendar.changeDate(lastdate);
    		var ddte=new Date(parseInt(el.attr("millis")));
    		ddte.setDate(ddte.getDate()-6);
    		calendar.changeDate(ddte);
    	}else{
