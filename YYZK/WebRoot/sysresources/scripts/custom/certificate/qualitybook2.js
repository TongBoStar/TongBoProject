		...
		$.webAjax({
			...
			//alert(data);
				...
				if(data.resultInfo[0]=="0")
				{
					...
					$('#reportdate').text(model.reportdate==null?"":model.reportdateString);
					$('#orderno').val(model.orderno==null?"":model.orderno);
					$('#customername').val(model.customername==null?"":model.customername);
 ...
 * @return
 */
function loadbody(type,itemcode){
		$('#itemcode').val(itemcode);
		var url=ctx+"/reportTec/report-tec!listToQuery.htm";
		$.webAjax({
			type:'post',
			url:url,
			data:'reportTecQuery.plandetailid='+$('#plandetailid').val(),
			success: function(data)
			{
				data = $.parseJSON(data);
				if(data.resultInfo[0]=="0"&&data.reportTecList.length>0)
				{
					...
					if(type==1){//全部
						...
						$("#orderno").val(model.orderno==null?"":model.orderno);	//	
						$("#customername").val(model.customername==null?"":model.customername);	//	
						$("#inpressMax").val(model.inpressMax==null?"":model.inpressMax);	//	
						$("#inpressMin").val(model.inpressMin==null?"":model.inpressMin);	//	
						$("#tecid").val(model.id==null?"":model.id);	//	
					}else if(type==6){//调节阀2
						$("#diameter").text(model.diameter==null?"":model.diameter);	//	公称直径
						$("#cpressure").text(model.cpressure==null?"":model.cpressure);	//	主阀压力等级
...
function savetec(type){//type:1:全部 2：noty 3:noqd
		...
		//	savestr+='&mindifference ='+$("#mindifference").text();//主阀最小进出口压差
			...
			savestr+='&leakage ='+$("#leakage").text();//泄漏等级
			savestr+='&aq='+$("#aq").val();
			savestr+='&plandetailid='+$("#plandetailid").val();
			savestr+='&orderno='+$("#orderno").val();
			savestr+='&customername='+$("#customername").val();
			savestr+='&inpressMax='+$("#inpressMax").val();
			savestr+='&inpressMin='+$("#inpressMin").val();
			savestr+='&itemcode='+$("#itemcode").val();
			if($('#tecid').val()!=""&&$('#tecid').val()!=null){
				savestr+='&id='+$("#tecid").val();
			}/**else{
				savestr+='&id=2';
			}*/
		}else if(type==6){//调节阀2
...
function loadzj(type,itemcode){
	...
	/**从历史物料中自动填写ac sg sz aq等参数*/
	$.webAjax({
		type:'post',
		url:ctx+'/reportTec/report-tec!showByItemcode.htm',
		data:'reportTecQuery.itemcode='+itemcode,
		success: function(data)
		{
			//data = $.parseJSON(data);
			if(null!=data&&data.length>0){
				...
				$('#sg').val(data[0].sg);
				$('#sz').val(data[0].sz);
				$('#inpressMin').val(data[0].inpressMin);
				$('#inpressMax').val(data[0].inpressMax);
				$('#overpressval').val(data[0].overpressval);
			}else{
