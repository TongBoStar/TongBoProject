$(document).ready(function() {
	window.simpleTable =$(document).SimpleTable({
		id:'productmanagelistpage',
		formId: 'pageForm',
		pageNo: '1',
		pageSize : '20',
		order: 'asc',
		orderBy: 't_productmanage_task.fstatus',
...
function search(){
	...
	var url = ctx+"/productmanageTask/productmanage-task!listToJson.htm";
	$.webAjax( {
		type : "post",
		url : url,
		...
		data = $.parseJSON(data);
		var page = data.page;
		$.refreshPageToolbar(page,"productmanagelistpage",simpleTable);
			if(data.resultInfo[0]=='0'){
				$.each(page.result,function(i,o){
					...
					html+='<td'+style+'>'+(o.productTaskCode==null?"":o.productTaskCode)+'</td>';
					html+='<td'+style+'><a href="#" onclick="toDetail('+o.id+','+o.sourceId+')">'+o.productCode+'</a></td>';
...
function toDetail(id,sourceId){
	window.location.href=ctx+"/productmanage/productmanagedetail.htm?id="+id+"&did="+sourceId;
}
/**
 * 关闭单据
 */
function toClose(){
	...
	if (flag) {
		openblockdiv();
		var url = ctx+"/productmanageTask/productmanage-task!closeToJson.htm";
		$.webAjax( {
			type : "post",
			url : url,
			data:param,
