...
function loaddesignassist(){
	openblockdiv();
	var url = ctx+"/storeroomTask/storeroom-task!showToJson.htm";
	$.webAjax({
		type : "post",
		url : url,
		data: 'storeroomTaskId='+$('#hiddenId').val(),
...
function saveexamine(type,mark){
	...
	var url = ctx+"/storeroomTask/storeroom-task!exeProcess.htm";
	var param = 'processExetask.sourceId='+$('#hiddenId').val()+
...
function loadexestatus(model){
	...
	var param = 'processExetaskQuery.sourceId='+$('#hiddenId').val();
	$.webAjax({
		type : "post",
		url : url,
		data: param,
			...
			if(data.resultInfo[0]=='0'){
				if(null!=data.processExetaskList&&data.processExetaskList.length>0){
					$.each(data.processExetaskList,function(i,o){
						if(o.taskCode=='TO_BRACKET_STOREROOM_TASK'){
						...
						}else if(o.taskCode=='TO_BAIT_STOREROOM_TASK'){
						...
						}else if(o.taskCode=='TO_ASSWELD_STOREROOM_TASK'){
						...
						}else if(o.taskCode=='TO_ASSEMBLE_STOREROOM_TASK'){
						...
						}else if(o.taskCode=='TO_DELIVERY_STOREROOM_TASK'){
...
function checkprocesshis(mark){
	...
	var param = 'processHistaskQuery.taskCode='+$('#taskcode_'+mark).val()+
				"&processHistaskQuery.sourceId="+$('#hiddenId').val();
	$.webAjax({
		type : "post",
		url : url,
		data: param,
...
function unexamine(type,modelmark){
	...
	if(flag){
		var url = ctx+"/storeroomTask/storeroom-task!unexamineToJson.htm";
		$.webAjax({
			type : "post",
			url : url,
			data: "storeroomTaskQuery.fmark="+modelmark+"&storeroomTaskQuery.id="+$('#hiddenId').val(),
			success : function(data) {
			data = $.parseJSON(data);
				if(data.resultInfo[0]=='0'){
					alert("反审成功!");
...
function checkchanges(sourceid,mark){
	var html='';
	var url = ctx+"/modelList/model-list!listToOutQuery.htm";
	$.webAjax( {
		type : "post",
		url : url,
		data: "modelListQuery.designassistId="+sourceid,
		success : function(data) {
		data = $.parseJSON(data);
			if(data.resultInfo[0]=='0'){
				$.each(data.modelListList,function(i,o){
					...
					html+='<td>'+(o.modifycount==null?0:o.modifycount)+'</td>';
					html+='<td>'+(o.manualfcount==null?0:o.manualfcount)+'</td>';
					html+='<td>'+(o.stocks==null?0:o.stocks)+'</td>';
					html += '</tr>';
				});
