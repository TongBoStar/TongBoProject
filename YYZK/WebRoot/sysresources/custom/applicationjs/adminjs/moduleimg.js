$(document).ready(function() {
	window.simpleTable =$(document).SimpleTable({
		id:'moduleimglistpage',
		formId: 'pageForm',
		pageNo: '1',
		pageSize : '20',
		order: 'asc',
		orderBy: 't_module_img.id',
...
function search(){
	...
	var url = ctx+"/moduleImg/module-img!listToJson.htm";
	$.webAjax( {
		type : "post",
		url : url,
		...
		data = $.parseJSON(data);
		var page = data.page;
		$.refreshPageToolbar(page,"moduleimglistpage",simpleTable);
			if(data.resultInfo[0]=='0'){
				$.each(page.result,function(i,o){
					...
					html+='<td>'+replaceNull(o.moduleImgName)+'</td>';
					html+='<td>'+replaceNull(o.imgPath)+'</td>';
					html+='<td>'+replaceNull(o.toUrl)+'</td>';
				});
				$('#data-body').html(html);
			}
			closeblockdiv();
...
function save(){
	...
	if (flag) {
		...
		var url = ctx+"/moduleImg/module-img!saveToJson.htm";
		$.webAjax( {
			type : "post",
			url : url,
			data:formParam,
...
function toEdit(){
	...
	var url = ctx+"/moduleImg/module-img!showToJson.htm";
	$.webAjax( {
		type : "post",
		url : url,
		data:"moduleImgId="+editid,
		success : function(data) {
			data = $.parseJSON(data);
			if(data.resultInfo[0]=='0'){
				$('#hiddenId').val(data.model.id);
				$('#moduleImgName').val(replaceNull(data.model.moduleImgName));
				$('#moduleId').val(data.model.moduleId);
				$('#toUrl').val(data.model.toUrl);
				$('#filepath').val(data.model.imgPath);
				openDiv('inputDiv');
			}
		}
	});
