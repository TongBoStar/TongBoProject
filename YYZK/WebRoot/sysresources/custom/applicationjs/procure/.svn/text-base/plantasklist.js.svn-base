...
function search(){
	...
	var url = ctx+"/planTask/plan-task!listToJson.htm";
	$.webAjax( {
		type : "post",
		url : url,
		...
		data = $.parseJSON(data);
		var page = data.page;
		$.refreshPageToolbar(page,"plantasklistpage",simpleTable);
			if(data.resultInfo[0]=='0'){
				$.each(page.result,function(i,o){
					...
					html+='<td'+style+'>'+(o.planTaskCode==null?"":o.planTaskCode)+'</td>';
					html+='<td'+style+'><a href="#" onclick="toDetail('+o.id+','+o.sourceId+')">'+(o.productCode==null?"":o.productCode)+'</a></td>';
					...
					html+='<td'+style+'>'+(o.planendtime==null?"":o.planendtime.substring(0,10))+'</td>';
...
function toDetail(id,sourceId){
	window.location.href=ctx+"/procure/plantaskdetail.htm?id="+id+"&did="+sourceId;
}
/**
 * 关闭单据
 */
function toClose(){
	...
	if (flag) {
		openblockdiv();
		var url = ctx+"/planTask/plan-task!closeToJson.htm";
		$.webAjax( {
			type : "post",
			url : url,
			data:param,
