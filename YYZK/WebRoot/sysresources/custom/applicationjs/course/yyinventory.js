$(document).ready(function() {
	window.simpleTable =$(document).SimpleTable({
		id:'yyinventorylistpage',
		formId: 'pageForm',
		pageNo: '1',
		pageSize : '20',
		order: 'asc',
		orderBy: 't_yyinventory.itemCode',
...
function search(){
	...
	var url = ctx+"/yyinventory/yyinventory!listToJson.htm";
	$.webAjax( {
		type : "post",
		url : url,
		...
		data = $.parseJSON(data);
		var page = data.page;
		$.refreshPageToolbar(page,"yyinventorylistpage",simpleTable);
			if(data.resultInfo[0]=='0'){
				$.each(page.result,function(i,o){
					...
					html+='<td>'+replaceNull(o.fqtyall)+'</td>';
...
function save(){
	...
	if (flag) {
		...
		var url = ctx+"/yyinventory/yyinventory!saveToJson.htm";
		$.webAjax( {
			type : "post",
			url : url,
			data:formParam,
...
function toEdit(){
	...
	$('#fbatchno').val("");
	$('#fmark').val("");
	var url = ctx+"/yyinventory/yyinventory!showToJson.htm";
	$.webAjax( {
		type : "post",
		url : url,
		data:"yyinventoryId="+editid,
		success : function(data) {
			data = $.parseJSON(data);
			if(data.resultInfo[0]=='0'){
...
function toDelete(id){
	...
	if (flag) {
		openblockdiv();
		var formParam = $('#inputForm').serialize();
		var url = ctx+"/yyinventory/yyinventory!deleteToJson.htm";
		$.webAjax( {
			type : "post",
			url : url,
			data:param,
