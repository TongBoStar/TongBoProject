...
$(function () {
    ...
    $(window).on("load resize", function () {
        ...
        if($('#sidebar').height() > $('#page-wrapper').height()){
            $('#wrapper').css('height', $('#sidebar').height()-40);
        }
    });
	...
	*/
    ...
    var setTheme = function (menu_style, style, header, color) {
            ...
            if ($('body').hasClass('sidebar-collapsed')) {
                ...
                setTimeout(function(){
                    $('#side-menu').slimScroll({
                        "height": $(window).height() - 50,//$(window).height() - 100
                        'width': '197px',
                        'wheelStep': 30
                    });
                    $('#side-menu').focus();
        ...
        $('#theme-change').attr('href', ctx+'/sysresources/css/themes/'+ style + '/' + color + '.css');
    }
    setTheme("head-fixed", "style1", /**list_header.val()*/'header-fixed', "orange-blue");
    ...
    if ($.cookie('style')) {
        ...
        if ($('body').hasClass('clear-cookie')) {
            ...
            setTheme($.cookie('menu_style'), /**$.cookie('style')*/'style1',/**$.cookie('header')*/'header-fixed', /**$.cookie('color')*/'orange-blue');
        }
    };
    ...
    list_menu.on('change', function(){
        ...
        if (($.cookie('header') == 'header-fixed') && ($(this).val() == 'sidebar-icons')) {
            setTheme($(this).val(), list_style.val(), /**'header-static'*/'header-fixed', color_active);
            return;
        }
        setTheme($(this).val(), list_style.val(), /**list_header.val()*/'header-fixed', color_active);
    });
    // SELECT STYLE EVENT
