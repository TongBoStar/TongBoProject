...
function saveAdmin(){
	...
	$.ajax({
		type: "post",
		url: "${ctx}/admin/admin!updateAdminByWxcode.htm",
		data:formParam,
		success: function (data) {
			data = $.parseJSON(data);
			if(data.resultInfo[0]=='0'){
...
function toprozhixiaoxieyi(){
	window.location.href = "${ctx}/mobile/saleagreement.jsp?openid=${param.openid}";
}
function checkpwd(){
	window.location.href = "${ctx}/mobile/checkpwd.jsp?adminId="+$('#hiddenAdminId').val()+"&openid=${param.openid}";
}
</script>
</head>
<body>
  ...
  <div class="w main">
  	<form id="frm_register" method="post" action="">
        <div class="page-container">
			...
			<input type="text" id="loginName" name="loginName" class="username" placeholder="用户名称" readonly="readonly">
			<input type="text" id="adminName" name="adminName" class="username" placeholder="真实姓名" readonly="readonly">
            ...
            <input type="text" id="email" name="email" class="username" placeholder="邮箱地址">
            <input type="text" id="zfbCode" name="zfbCode" class="username" placeholder="支付宝账号">
            <input type="text" id="wxCard" name="wxCard" class="username" placeholder="微信账号">
            <input type="text" id="ylCard" name="ylCard" class="username" placeholder="银联卡号">
            <input type="text" id="ylCardType" name="ylCardType" class="username" placeholder="银联卡所属银行">
        </div>
      </form>
      <div class="item item-captcha" style="margin-top:5px;">
          <div class="err-tips"><a href="javascript:void(0);" onclick="openlayer1();">查看云趣家成员营业守则</a> </div>
          <div class="err-tips"><a href="javascript:void(0);" onclick="toprozhixiaoxieyi();">查看产品直销协议</a> </div>
        </div>
      <div class="ui-btn-wrap"><button type="button" onclick="saveAdmin();">保存</button> </div>
      <div class="ui-btn-wrap"><button type="button" onclick="checkpwd();">修改密码</button> </div>
  </div>
  </div>
</div>
